...
11. User can make one-time secure payment for using the website.   
    11.1. Expected: if user hasn't paid yet and 3 recipes were created, on the 4th recipe creation s/he will be asked to pay.    
    11.1. Testing. 1. Precondition: user hasn't paid yet. Create 3 recipes. -> The recipes were created. No payment was required. 2. Start creating the 4th recipe. -> Instead of Add a new recipe form, the Make one-time payment popup is displayed.     
    ![Payment popup](static/img/readme/testing/payment-modal.png)     
    11.2. Expected: it's not possible to by pass the Make one-time payment popup.    
    11.2. Testing. 1. Try to click outside of the popup -> The popup stays open, the form is not reachable. 2. Try to press the Escape key -> The popup stays open, the form is not reachable. 3. Try to reload the Add a new recipe form. -> The popup appears and blocks the form editing.    
    11.3. Expected: for making payment user is redirected to the 3d party payment system (Stripe).    
    11.3. Testing. From the Make one-time payment popup click the Make Payment button -> The Stripe checkout page is open where user is asked to pay 25 euro fees.     
    ![Stripe checkout](static/img/readme/testing/stripe-checkout.png)   
    11.4. Expected: user can cancel the payment process.    
    11.4. Testing. From the Stripe checkout page click the back button (can be found close to the fees) -> The checkout was cancelled. User gets the corresponding message and the link to the Home page.    
    ![Cancelled checkout](static/img/readme/testing/failed-checkout.png)         
    11.5. Expected: user cannot complete the checkout with invalid credit card number or without filling in mandatory fields.      
    11.5. Testing. 1. On the Stripe checkout page enter invalid credit card number (from the Stripe website: 1234 5679 0123 4567) -> The checkout page indicates 'Your card number is invalid'. The Pay button is disabled. 2. On the Stripe checkout page enter valid credit card number (from the Stripe website: 4242 4242 4242 4242) but don't add the card holder's name/CVC/expiry date. -> The card number accepted but without entering the card holder's name/CVC/expiry date user cannot proceed.     
    11.6. Expected: user can finish the checkout successfully when provides valid credit card data.        
    11.6. Testing. 1. On the Stripe checkout page enter valid credit card number (from the Stripe website: 4242 4242 4242 4242) and fill in all required fields. -> Pay button becomes active. 2. Click on the Pay button. -> User is sent to the Successful payment page.      
    ![Successful checkout](static/img/readme/testing/successful-payment-page.png)       
    11.7. Expected: if checkout token doesn't match account token, the checkout must fail.    
    11.7. Testing. Using the successful checkout url from the previous test, change the token in it and hit the url. -> The Failure page will be shown to the user.   
    11.8. Expected: once user paid, he can continue creating new recipes without being asked to pay again.    
    11.8. Testing. With the same user tested in the upper scenarios, return to the Add new recipe form and create the 4th recipe. -> When the user opens the form, no popup blocks the form and s/he can add a new recipe without issues.    
12. Registered user's recipes are not visible to an anonymous user.    
    12.1. Expected: registered user's recipes are not visible to an anonymous user.    
    12.1. Testing. Prepare 3 urls: My Recipes page, Edit and View recipe pages. Make sure user is logged out and hit each of the copied urls. -> All urls will lead to the Login page.    
13. user can access the website from multiple devices of different sizes without major UI issues.    
    13.1. Expected: every page of the website should display without issues; user should be able to easily navigate through My Recipe Book; creation/viewing/editing/deletion must be possible to do from the mobile device.    
    13.1. Testing. 1. Open the Home page -> the page is opened, the navigation links are hidden but instead there is the Menu button. When clicked, all links are present in the expanded menu. 2. scroll down the Home page -> Add a new recipe section and Courses section are visible. Courses cards/links are displayed in 1 column. 3. Open the All recipes page with multiple recipes created -> all recipes are displayed in 1 column. 4. Create/view/edit/delete a recipe -> each action can be performed successfully. 5. The checkout process can be completed successfully.     
14. Other links/navigation testing.     
    14.1. Expected: user clicks the nav and social media links, s/he is redirected to the right pages/sections.     
    14.1. Testing. 1. Click on the Home nav item -> the Home page is refreshed. 2. Click on the logo -> the Home page is refreshed. 3. Click on the My Recipes nav item -> All Recipes page is opened either with recipe cards or 'You haven't added any recipes yet' text and the link to add a new recipe. 4. Click on the facebook/instagram/twitter icons -> facebook/instagram/twitter page is opened in a separate tab.        
15. Extra UI testing for the My Recipes page on laptop device.     
    15.1. Expected: when many recipes were created on the page, the cards layout displays without major issues.        
    15.1 Testing. 1. Create more than 4 recipes (for desktop) -> the recipes are displayed in 4 columns. They have the same size: height and width regardless difference in title length. Long titles are shown with ellipsis. 2. Create many recipes to check the list is scrollable -> user can scroll without issues.      
    15.2. Expected: when user upload an image in landscape orientation, it should look good.     
    15.2. Testing. Add/edit a recipe with uploading an image in landscape orientation and save the changes. -> The recipe card shows the full recipe image and it's resizable when changing the screen resolution.    
16. Page not found.     
    16.1. Expected: when user hits a non-existing url, then the styled 404 page will be displayed to her/him.    
    16.2. Testing. Mistype an existing url and press enter. -> 404 page is shown giving a hint this url doesnt exist.    
    ![404 page](static/img/readme/testing/404-page.png)     